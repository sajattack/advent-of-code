...
fn get_cookie() -> Result<String> {
    ...
    let cookie: String; 
    if args.len() != 2 {
        return err!("You must specify your adventofcode session cookie as the first and only argument")
    } else {
       cookie = args[1].clone(); 
    }
    Ok(cookie)
...
pub fn submit_answer(day: u8, level: u8, answer: String) -> Result<String> {
    ...
    let html = client.post(
        &format!("https://adventofcode.com/2018/day/{}/answer", day))
        .header(COOKIE, format!("session={}", cookie).as_str())
        .form(&data)
        .send()?.text()?;
